...
class PiratePigGame extends Sprite {
	...
	private function construct ():Void {
		...
		gameInput.addEventListener (TouchEvent.TOUCH_BEGIN, this.crashMe);
		#end
		gameInput.addEventListener (TouchEvent.TOUCH_BEGIN, this.crashMe);
	...
	private function crashMe(event:TouchEvent):Void {
		trace("let's crash this thing!");
		lime.utils.Log.warn("let's crash this thing");
		// stackOverflow(1);
		var thisisnull = null;
		thisisnull.toString();
	}

	private function stackOverflow(count:Int) {
		trace(count + " frames on the stack.");
		stackOverflow(count);
	}
